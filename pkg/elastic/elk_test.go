...
func TestElasticImpl_Information(t *testing.T) {
	t.Skip("elasticsearch required")

	es, err := NewElastic(testElasticHosts)
	require.NoError(t, err)

	r, _, err := es.Search(ctx, "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", 0, 1, false)
	require.NoError(t, err)
	require.Len(t, r, 1)
	...
	r, _, err = es.Search(ctx, "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", 1, 1, false)
	require.NoError(t, err)
	require.Len(t, r, 1)

	r, _, err = es.Search(ctx, "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", 2, 1, false)
	require.NoError(t, err)
	require.Len(t, r, 1)

	r, _, err = es.Search(ctx, "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", 30, 1, false)
	require.NoError(t, err)
	require.Len(t, r, 0)
}

func TestElasticImpl_SearchCount(t *testing.T) {
	t.Skip("elasticsearch required")

	es, err := NewElastic(testElasticHosts)
	require.NoError(t, err)

	r, err := es.SearchCount(ctx, "onny", false)
	require.NoError(t, err)
	require.Equal(t, r, 0)

	r, err = es.SearchCount(ctx, "BonnyObnaI", false)
	require.NoError(t, err)
	require.Greater(t, r, 0)
}

func TestElasticImpl_SearchCountWildcard(t *testing.T) {
	t.Skip("elasticsearch required")

	es, err := NewElastic(testElasticHosts)
	require.NoError(t, err)

	r, err := es.SearchCount(ctx, "onny", true)
	require.NoError(t, err)
	require.Equal(t, r, 408)

	r, err = es.SearchCount(ctx, "BonnyObnaI", true)
	require.NoError(t, err)
	require.Equal(t, r, 3)
}
